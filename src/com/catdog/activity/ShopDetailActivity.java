...
import com.catdog.adapter.ProductDetailGridAdapter;
...
import com.catdog.view.ActionItem;
import com.catdog.view.MyGridView;
import com.catdog.view.MyViewPager;
import com.catdog.view.Options;
import com.catdog.view.TitlePopup;
import com.catdog.view.TitlePopup.OnItemOnClickListener;
...
public class ShopDetailActivity extends BaseActivity implements OnClickListener {
	...
	private TextView tv_title, tv_price, tv_guige;
	private TextView tv_kucun;
	private Button img_reduce, img_add;
	...
	private ProductDetailGridAdapter mAdapter;
	private List<Recordsets> mList;
	...
	private void requestGanxingqu(int currenpage) {
		...
		HttpUtils.getDataFromVolley(mContext, URLConstant.TWO_CLASSIFY, null,
				map, twoListener);
	...
	private ResponseListener twoListener = new ResponseListener() {
		...
		public void getResponse(Object object) throws JSONException {
			...
			ClassifyListBean bean = GsonUtil.jsonToBean(str,
	...
	private void initView() {
		...
		ll_pic = (Button) findViewById(R.id.shopDetail_ll_pic);
		img_reduce = (Button) findViewById(R.id.detail_img_reduce);
		img_add = (Button) findViewById(R.id.detail_img_add);
		...
		btn_add = (Button) findViewById(R.id.btn_shop_detail_add_cart);
		btn_buy = (Button) findViewById(R.id.btn_shop_detail_buy);
		tv_num = (TextView) findViewById(R.id.detail_tv_num);
		btnComment = (Button) findViewById(R.id.detail_ll_comment);

		mGridView = (MyGridView) findViewById(R.id.shopDetail_gv);
		mGridView.setNumColumns(3);
		mGridView.setOnItemClickListener(gridviewClickListener);
		btnBack = (Button) findViewById(R.id.btn_product_detail_back);
		sv = (ScrollView) findViewById(R.id.sv_shop_detail);
		btnShare = (Button) findViewById(R.id.btn_product_detail_share);
	...
	private void initData() {
		mList = new ArrayList<Recordsets>();
		mAdapter = new ProductDetailGridAdapter(this, mList, bitmapUtils);
		mGridView.setAdapter(mAdapter);
		// 给标题栏弹窗添加子类
		titlePopup.addAction(new ActionItem(this, "收藏",
	...
	private void requestShopDeatil() {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("n_id", shopId);
		HttpUtils.getDataFromVolley(mContext, URLConstant.SHOP_DETAIL, null,
				map, detailListener);
	}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.btn_shop_detail_add_cart:
			if (!bean.getN_ding().equals("0")) {
				...
				try {
					...
					values.put(
							"shopcart_pic",
							"http://www.wssxls.com:80/shop/data/images/"
									+ bean.getN_pic());
					values.put("shopcart_focus", bean.getN_focus_key()
							.toString());
					...
					if (count > 0) {
						Toast.makeText(ShopDetailActivity.this, "添加至购物车成功",
								Toast.LENGTH_SHORT).show();
					} else {
						for (int j = 0; j < shopCartBeans.size(); j++) {
							if (shopCartBeans.get(j).getGoods_id()
									.equals(bean.getN_id())) {
								shopcart_number = Integer.valueOf(shopCartBeans
										.get(j).getShopcart_number());
							}
						}
						String sql = "update cart set shopcart_number = "
								+ (shopcart_number + num)
								+ " where goods_id = " + bean.getN_id();
			...
			break;
		case R.id.btn_shop_detail_buy:// 立即购买
			if (!bean.getN_ding().equals("0")) {
		...
		case R.id.detail_img_reduce:
			if (num != 1) {
				...
				Toast toast = Toast.makeText(mContext, "受不了了,宝贝不能再减少了哦!",
						Toast.LENGTH_SHORT);
		...
		case R.id.shopDetail_title_text:
			finish();
			break;
		case R.id.btn_product_detail_share:
			showPopWindow(v);
		default:
			break;
		}
	...
	private void requestDeleteCollect() {
		...
		HttpUtils.getDataFromVolley(mContext, URLConstant.DELETE_COLLECT, null,
				map, deleteListener);
	}
	...
	private void requestAddCollect() {
		...
		map.put("sc_meid", getUserId());
		map.put("sc_nid", shopId);
		HttpUtils.getDataFromVolley(mContext, URLConstant.ADD_COLLECT, null,
				map, addCollectListener);
	}
	...
	public List<ShopCartBean> getTotalist() {
		...
		while (cursor.moveToNext()) {
			...
			shopCartBean.setGoods_id(cursor.getString(cursor
					.getColumnIndex("goods_id")));
			shopCartBean.setShopcart_number(cursor.getString(cursor
					.getColumnIndex("shopcart_number")));
	...
	private void initHeadView() {
		...
		imageLoader.init(ImageLoaderConfiguration
				.createDefault(ShopDetailActivity.this));
		...
		imageComicAdapter = new ImageBannerDetailAdapter(imageList,
